 ...
 * @param {boolean} options.includeQuotes - Include "repost with thoughts" (quote) posts
 * @param {boolean} options.isSimpleRepost - Include simple reposts without comments
 * @returns {Array} Filtered array of own posts
 */
export function filterOwnPosts(items, username, options = {}) {
    const { includeQuotes = false, isSimpleRepost = false } = options;
    ...
    return items.filter(item => {
        ...
        // Regular posts are always included
        if (item.post_type === 'regular') return true;
        
        // Quote posts (repost with thoughts) included if option is enabled
        if (includeQuotes && item.post_type === 'quote' && item.reshared_post) {
            return true;
        }

        // Simple reposts included if option is enabled
        // A simple repost is when user shares content without adding their own text
        if (isSimpleRepost && item.post_type === 'repost' && item.reshared_post) {
            return true;
        }
