...
export async function fetchLinkedInPosts(client, username, useMock = false) {
    ...
    // (some items may be comments, others' posts, etc.)
    const maxCards = parseInt(process.env.MAX_CARDS_TO_GENERATE || '4', 10);
    const listLimit = Math.min(maxCards * 3, 25); // Request 3x buffer, max 25
    const { items } = await dataset.listItems({ limit: listLimit });
    console.log(`Fetched ${items.length} items from dataset (requested ${listLimit})`);
...
export function filterOwnPosts(items, username, includeReposts) {
    ...
    return items.filter(item => {
        ...
        if (item.post_type === 'regular' && item.author.username === username) {
            return true;
        }
        
        // For reposts (quote or simple), include if option is enabled
        // The author is the user who shared it, regardless of original author
        if (includeReposts && 
            (item.post_type === 'quote' || item.post_type === 'repost') &&
