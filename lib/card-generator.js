...
import { translateRelativeTime, translateRepostLabel } from './translations.js';
...
export async function generateCard(post, outputDir, templateDir, options = {}) {
    ...
    for (const theme of themes) {
        ...
        template = template.replace(/\$\{name\}/g, escapeXML(name));
        template = template.replace(/\$\{bio\}/g, escapeXML(bio));
        template = template.replace(/\$\{profile_picture\}/g, profilePicture);
        
        // Escape XML and replace newlines with <br/> for proper formatting
        const safeText = escapeXML(text).replace(/\n/g, '<br/>');
        template = template.replace(/\$\{text\}/g, safeText);
        
        template = template.replace(/\$\{time\}/g, escapeXML(time));
        template = template.replace(/\$\{reactions\}/g, reactions);
        template = template.replace(/\$\{comments\}/g, escapeXML(comments));

        // If this is a simple repost, replace the `${reposts}` placeholder
        // with a language-aware string (falls back to sensible defaults).
        if (isSimpleRepost) {
            const repostsText = translateRepostLabel(language, translations);
            template = template.replace(/\$\{reposts\}/g, escapeXML(repostsText));
        }
        
        // Replace original post placeholders for quote posts and simple reposts
        if (isQuotePost || isSimpleRepost) {
            template = template.replace(/\$\{original_name\}/g, escapeXML(originalName));
            template = template.replace(/\$\{original_bio\}/g, escapeXML(originalBio));
            template = template.replace(/\$\{original_profile_picture\}/g, originalProfilePicture);
            
            const safeOriginalText = escapeXML(originalText).replace(/\n/g, '<br/>');
            template = template.replace(/\$\{original_text\}/g, safeOriginalText);
