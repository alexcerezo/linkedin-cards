...
export async function generateCard(post, outputDir, templateDir, options = {}) {
    ...
    for (const theme of themes) {
        ...
        template = template.replace(/\$\{name\}/g, escapeXmlText(name));
        template = template.replace(/\$\{bio\}/g, escapeXmlText(bio));
        template = template.replace(/\$\{profile_picture\}/g, profilePicture);
        template = template.replace(/\$\{text\}/g, escapeXmlText(text));
        template = template.replace(/\$\{time\}/g, escapeXmlText(time));
        template = template.replace(/\$\{reactions\}/g, reactions);
        template = template.replace(/\$\{comments\}/g, escapeXmlText(comments));
        ...
        if (isSimpleRepost) {
            ...
            template = template.replace(/\$\{reposts\}/g, escapeXmlText(repostsText));
        }
        
        // Replace original post placeholders for quote posts and simple reposts
        if (isQuotePost || isSimpleRepost) {
            template = template.replace(/\$\{original_name\}/g, escapeXmlText(originalName));
            template = template.replace(/\$\{original_bio\}/g, escapeXmlText(originalBio));
            template = template.replace(/\$\{original_profile_picture\}/g, originalProfilePicture);
            template = template.replace(/\$\{original_text\}/g, escapeXmlText(originalText));
