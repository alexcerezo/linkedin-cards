...
export async function generateCard(post, outputDir, templateDir, options = {}) {
    ...
    // Prepare original post data for quote posts
    let originalName = '';
    let originalBio = '';
    ...
    for (const theme of themes) {
        ...
        let templateName;
        if (isQuotePost) {
            // Use quote-media template if original post has media, otherwise regular quote
            templateName = originalHasMedia 
                ? `linkedin-post-quote-media-${theme}.svg`
                : `linkedin-post-quote-${theme}.svg`;
        } else if (hasMedia) {
            templateName = `linkedin-post-${theme}.svg`;
        } else {
            templateName = `linkedin-post-${theme}-text.svg`;
        }
        ...
        // Replace original post placeholders for quote posts
        if (isQuotePost) {
